...
Logger logger = Logger.getLogger("showRatingUserSection3.jsp");
String redirect_url="showRatingUserSection3?clr=app_section3&act=app_section3";
%>
	<%
...
<%@ include file="adminHeader.jsp" %>
    ...
    try
    {
    	ArrayList<UserRatingBeans> userRatingBeanss=new ArrayList<UserRatingBeans>();
                    int count =0;
                    if(searchValue.equalsIgnoreCase("0")){
                    	String sql="SELECT user.username,user_rating_section3.*,judges.NAME AS judges_name  FROM user_rating_section3 INNER JOIN user ON user.id=user_rating_section3.userid INNER JOIN judges ON judges.id=user_rating_section3.judgeid WHERE user_rating_section3.userid="+userid+" order by user_rating_section3.id desc limit "+page1+","+order2+"";
                    	userRatingBeanss=adminDaoImpl.getUserRatingSection1(sql);
                    }else{
                    	String sql="SELECT user.username,user_rating_section3.*,judges.NAME AS judges_name  FROM user_rating_section3 INNER JOIN user ON user.id=user_rating_section3.userid INNER JOIN judges ON judges.id=user_rating_section3.judgeid WHERE user_rating_section3.userid="+userid+" and (user_rating_section3.id  LIKE '"+searchValue+"%' or user_rating_section3.judgeid LIKE '"+searchValue+"%' or user_rating_section3.rating LIKE '"+searchValue+"%') order by user_rating_section3.id desc limit "+page1+","+order2+"";
                    	userRatingBeanss=adminDaoImpl.getUserRatingSection1(sql);
                    }
			 ...
			 try
			    {
                    int count=0;
                    if(searchValue.equalsIgnoreCase("0")){
                    	String sql="SELECT COUNT(*)  FROM user_rating_section3 WHERE userid="+userid+"";
                    	count=adminDaoImpl.getCountBySql(sql);
                    }else{
                    	String sql="SELECT COUNT(*)  FROM user_rating_section3 WHERE userid="+userid+" and (user_rating_section3.id  LIKE '"+searchValue+"%' or user_rating_section3.judgeid LIKE '"+searchValue+"%' or user_rating_section3.rating LIKE '"+searchValue+"%')";
                    	count=adminDaoImpl.getCountBySql(sql);
                    }
                    	double count1=count;
                    	total_rows=count;
